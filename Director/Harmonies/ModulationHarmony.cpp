...
namespace Music
{
    ModulationHarmony::ModulationHarmony(const std::string& port, int channel, int modulationOffset):
                       ...
                       Harmony(port, channel)
    {}

    MIDI::NoteMessagePointer ModulationHarmony::generate(const MIDI::NoteMessagePointer& noteMessage)
    {
        ...
        {
            modulatedNote = theory->modulate(noteOn->note, modulationOffset);
